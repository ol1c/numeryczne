...
% y_mix - wartości double wielomianu określonego przez double(coef_vpa)
    ...
    hold on
    % TODO
    plot(x_fine, y_double, 'r--', 'LineWidth', 1, 'DisplayName', 'Interpolacja');
    ...
    hold on
    % TODO
    plot(x_fine, y_vpa, 'b--', 'LineWidth', 1, 'DisplayName', 'Interpolacja');
    ...
    hold on
    % TODO
    plot(x_fine, y_mix, 'g--', 'LineWidth', 1, 'DisplayName', 'Interpolacja'); 
    
    subplot(3,1,1);
    title('Interpolacja funkcji Rungego (double)');
    xlabel('x');
    ylabel('f(x)');
    legend('Location', 'eastoutside');
    
    subplot(3,1,2);
    title('Interpolacja funkcji Rungego (vpa)');
    xlabel('x');
    ylabel('f(x)');
    legend('Location', 'eastoutside');

    subplot(3,1,3);
    title('Interpolacja funkcji Rungego (vpa -> double)');
    xlabel('x');
    ylabel('f(x)');
    legend('Location', 'eastoutside');
