function [ft_5, integral_1000, Nt, integration_error] = midpoint_rule_accuracy_evaluation()
    ...
    for i=1:length(Nt)
        ...
        integration_error(i) = abs(midpoint_rule(x) - reference_value);
    end
    loglog(Nt, integration_error);
    xlabel("ilość przedziałów");
    ylabel("błąd");
    title("wartość błędu całkowania metodą prostokątów");
...
function integral_approximation = midpoint_rule(x)
    ...
    integral_approximation = sum(failure_density_function(midpoints) .* delta);
end
